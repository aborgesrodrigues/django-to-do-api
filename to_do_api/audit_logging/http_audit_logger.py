...
class HTTPAuditLogger(threading.Thread):
    ...
    def log_request(self, req: HttpRequest):
        try:
    ...
    def log_response(self, req: HttpRequest, resp: HttpResponse, include_request_in_response: bool,
    ...
    def _queue_record(self, audit_id: str, data: dict, req_data: t.Optional[HttpRequest]):
    ...
    @staticmethod
    def _get_request_metadata(req: HttpRequest) -> dict:
        metadata = {
            "host": req.get_host(),
            ...
            "query": str(req.GET),  # convert to string
            "headers": list(req.headers),
        }

        if req.META.get('CONTENT_LENGTH'):
            metadata["body"] = HTTPAuditLogger._request_body(req)

        return metadata

    @staticmethod
    def _get_response_metadata(req: HttpRequest, response: HttpResponse, include_request_in_response: bool,
                               request_timestamp: t.Optional[str]) -> dict:
        metadata = {
            "requestHost": req.get_host(),
            "requestHostname": req.get_host(),
    ...
    @staticmethod
    def _make_audit_id(req: HttpRequest, is_response: bool) -> str:
        """
    ...
    def log_inbound(self, log_request=True, log_response=True,
        ...
        def _log_inbound(f):
            @wraps(f)
            def __log_inbound(view: viewsets.ModelViewSet, request: HttpRequest, *args, **kwargs):
                request_timestamp = None
                if include_request_in_response:
                    request_timestamp = _utc_now_str()
                if log_request:
                    self.log_request(req=request)
                result = f(view, request, *args, **kwargs)
                if log_response:
